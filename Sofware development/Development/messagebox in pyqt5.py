from PyQt5.QtWidgets import QApplication,QMenuBar,QMainWindow,QToolBar,QFileDialog,QTextEdit,QMessageBox,QFontDialog,QColorDialog,QAction
import sys
...
class mywindow(QMainWindow): #child class
    ...
    def createmenu(self): #class method
        ...
        saveaction.triggered.connect(self.saveas); #connection a signal to a slot
        ...
        aboutAction=QAction(QtGui.QIcon('About.png'), 'About Application', self); #create an action
        aboutAction.triggered.connect(self.aboutmessage); #connect a signal to a slot
        helpmenu.addAction(aboutAction); #add action to menubar

        choiceAction=QAction(QtGui.QIcon('Choice.png'), 'Your Mind', self); #create an action
        choiceAction.triggered.connect(self.choicemessage); #connect a signal to a slot
        helpmenu.addAction(choiceAction); #add action to menubar
    ...
    def saveas(self): #class method

        fn, _=QFileDialog.getSaveFileName(self,'Export pdf',None,'Pdf files (.pdf);;All Files()'); #get the filename in filedialog
        if fn !='':
    ...
    def aboutmessage(self): #class method
        message=QMessageBox.about(self,'About Application','This is a PyQt5 Application'); #create an about messagebox

    def choicemessage(self): #class method
        message=QMessageBox.question(self,'User Choice','Do You Like PyQt5 Application ?',QMessageBox.Yes | QMessageBox.No); #create a choice messagebox

        if message==QMessageBox.Yes:
            self.editor.setText('User likes PyQt5 Application'); #settext of editor
        elif message==QMessageBox.No:
            self.editor.setText('User does not like PyQt5 Application');#setText of editor
